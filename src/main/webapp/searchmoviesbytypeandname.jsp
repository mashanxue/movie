...
<div align="center">
    <form action="allMoviesByType">
        ...
        <option value='1' <c:if test='${list.get(0).movieTypeId == "1"}'> selected='selected' </c:if>>悬疑</option>
        <option value='2' <c:if test='${list.get(0).movieTypeId  == "2"}'> selected='selected' </c:if>>动作</option>
        <option value='3' <c:if test='${list.get(0).movieTypeId  == "3"}'> selected='selected' </c:if>>爱情</option>
        <option value='4' <c:if test='${list.get(0).movieTypeId  == "4"}'> selected='selected' </c:if>>科幻</option>
    </select>
        ...
        <c:when test="${fn:length(list)>1}">
            <c:choose>
                <c:when test="${pageInfo.size > 0 }">
                    ...
                    <a href="allMoviesByType?PageNum=1&movieTypeId=${list.get(0).movieTypeId}&movieName=${list.get(0).movieName}"><input type="button" value="首页" style="width:60px;height:40px;background-color:#f47920;border-radius: 50%;font-weight: 700;"></a>
                    <c:choose>
                        <c:when test="${pageInfo.hasPreviousPage }">
                            <a href="allMoviesByType?PageNum=${pageInfo.pageNum-1}&movieTypeId=${list.get(0).movieTypeId}&movieName=${list.get(0).movieName}" style="color:#f47920;" >上一页 </a>
                        </c:when>
                        <c:otherwise>
                    ...
                    <c:forEach var="item" items="${pageInfo.navigatepageNums}">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a href="allMoviesByType?PageNum=${item}&movieTypeId=${list.get(0).movieTypeId}&movieName=${list.get(0).movieName}">${item}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:choose>
                        <c:when test="${!pageInfo.isLastPage }">
                            <a href="allMoviesByType?PageNum=${pageInfo.pageNum+1}&movieTypeId=${list.get(0).movieTypeId}&movieName=${list.get(0).movieName}" style="color:#f47920;" >下一页 </a>
                            <a href="allMoviesByType?PageNum=${pageInfo.lastPage}&movieTypeId=${list.get(0).movieTypeId}&movieName=${list.get(0).movieName}" ><input type="button" value="尾页" style="width:60px;height:40px;background-color:#f47920;border-radius: 50%;font-weight: 700;"></a>
                        </c:when>
                        <c:otherwise>
...
<form action="updateMovies" method="post" align="center">
    <p align="center">
        ...
        电影价格:<input type="text" name="moviePrice" id="moviePrice">
        电影名字:<input type="text" name="movieName" id="movieName">
        电影时长:<input type="text" name="movieTime" id="movieTime">
    </p>
    <p align="center">
        电影地区:<input type="text" name="movieAddress" id="movieAddress">
        电影类别:<input type="text" name="movieTypeId" id="movieTypeId">
        电影评分:<input type="text" name="movieScore" id="movieScore">

    </p>
    <p align="center" vertical-align="top">
        电影主演:<input type="text" name="movieActor" id="movieActor">
        电影海报:<input type="text" name="moviePname" id="moviePname">
        电影摘要:<textarea style="vertical-align:text-top" rows="10" cols="23" name="movieAbstract" id="movieAbstract"> </textarea>
    </p>
